...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if ([self.backgroundViews count] > index) {
        ...
        if (v) {
            //            [v setAlpha:alpha != 1 ? 0 : 1];
            CGFloat viewX = 0 - (v.frame.size.width - v.frame.size.width * alpha);
            viewX = (index == 0 && viewX > 0) ? 0 : viewX;
            viewX = (viewX > 0) ? 0 : viewX;
            NSLog(@"viewX = %f", viewX);
            
            v.frame = CGRectMake(viewX, v.frame.origin.y, v.frame.size.width, v.frame.size.height);
            
            if (index < [self.backgroundViews count] - 1) {
                UIView *nextView = [self.backgroundViews objectAtIndex:index + 1];
                nextView.frame = CGRectMake(v.frame.size.width + viewX, nextView.frame.origin.y, nextView.frame.size.width, nextView.frame.size.height);
            }
        }
    }
